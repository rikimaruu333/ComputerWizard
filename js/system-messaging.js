...
$(document).ready(function() {
    let messageFetchInterval; // Variable to store the interval ID
        ...
        success: function(response) {
            if (response.status === 'success') {
                    ...
                    const freelancerDetails = `
                        ...
                        <div class="freelancer-button">
                          <div class="freelancer-button-details">
                              <i class="fas fa-user-circle" id="viewUserProfile"></i>
                              <span>Profile</span>
                          </div>
                        </div>
                ...
                if (messageFetchInterval) {
                  ...
                  // Set a new interval to fetch chat messages
  ...
  function fetchChatMessages(incomingId) {
    $.ajax({
      ...
      success: function(response) {
            $('.chat-box').empty();  // Clear the chat box before displaying messages

            if (response.error) {
                // Display the error message directly in the chat-box
                $('.chat-box').append('<div class="no-message">' + response.error + '</div>');
            } else {
